...
int main(){
    ...
    for(int i = 0; i<n;i++){
        ...
        sum_clock = sum_clock + (end - begin);
    }
    time_spent = (double)(sum_clock/n)/CLOCKS_PER_SEC;
    printf("\nNumero de interacoes: %d\n", n);
    printf("Tempo levou foi de %f\n",time_spent);
    printf("checksum irrelevante para nao permitir otimizacao total: %d %d %d\n", (int)param_1, (int)param_2, param_3);
    return 0;
}